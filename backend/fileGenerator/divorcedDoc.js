...
async function generateDoc({ form }) {
  ...
  const image = Media.addImage(
    docGenerated,
    fs.readFileSync(path.join() + "/pictures/ActeDAvocat.png")
  );
  ...
  renderBigTitle(
    doc,
    "CONVENTION DE DIVORCE PAR CONSENTEMENT MUTUEL CECI EST UN TETST"
  );

  renderItallicBoldText(doc, "Articles 229 et suivants du Code civil test");
...
function renderBigTitle(doc, title) {
  doc.push(
    new Paragraph({
      children: [
        new TextRun({
...
function renderCenterBoldText(doc, title) {
  doc.push(
    new Paragraph({
      children: [
        new TextRun({
...
function renderImage(doc, image) {
  doc.push(
    new Paragraph({
      children: [image],
      alignment: "center",
